...
import ArticleContent from '../Index/ArticlePageContent.vue'
import {getArticle} from 'assets/js/http.js'
const MarkdownIt = require('markdown-it')
const md = new MarkdownIt({
	html: false,//是否在源码中使用html标记
...
export default {
	...
	created: async function(){
		...
		getArticle(id).then(function(res){
			vm._loading = false;
			// 从响应数据中提取相关数据
			vm.mdFile = res.data.content.md;
			vm.article.id = res.data.content.id;
			vm.article.time = res.data.content.time;
			// 使用MD解析器解析MD文档为HTML文档
			vm.article.content = md.render(vm.mdFile);
		}).catch(function(err){
			console.log(err);
		});
	},
